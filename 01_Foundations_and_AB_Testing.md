...
3. [Core / North-Star Metrics (北极星指标)](#3-core--north-star-metrics)
4. [The Recommendation Pipeline / Link (推荐系统的链路)](#4-the-recommendation-pipeline--link)
   - [4.1 Retrieval (召回)](#41-retrieval)
   - [4.2 Pre-Rank (粗排) & Rank (精排)](#42-pre-rank--rank)
   - [4.3 Re-Rank (重排)](#43-re-rank)
   - [4.4 The Funnel Intuition (漏斗)](#44-the-funnel-intuition)
5. [The Experiment Workflow (实验流程)](#5-the-experiment-workflow)
6. [A/B Testing (A/B 测试)](#6-ab-testing)
   - [6.1 Treatment vs. Control & Random Bucketing](#61-treatment-vs-control--random-bucketing)
   - [6.2 Layered Experiments (分层实验)](#62-layered-experiments)
   - [6.3 Mutual Exclusion vs. Orthogonality (互斥 vs 正交)](#63-mutual-exclusion-vs-orthogonality)
   - [6.4 Holdout Mechanism (Holdout 机制)](#64-holdout-mechanism)
   - [6.5 Full Rollout & Reverse Experiments (实验推全 & 反转实验)](#65-full-rollout--reverse-experiments)
7. [Interview Quick-Reference](#7-key-insights)
...
A/B testing also serves **hyperparameter selection** — e.g. run three groups for GNN depth $\in \{1, 2, 3\}$ simultaneously and pick the best.

---

## 6. A/B Testing

Running example throughout: the retrieval team built a **GNN (graph neural network, 图神经网络) retrieval channel**, offline results are positive, and we now A/B test it online — both to measure its business-metric lift and to tune GNN depth $\in \{1,2,3\}$.
