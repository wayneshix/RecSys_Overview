...
1. [Why Serving Is the Binding Constraint](#1-why-serving-is-the-binding-constraint)
   - [1.1 The per-stage latency budget](#11-the-per-stage-latency-budget)
   - [1.2 The candidate-count asymmetry](#12-the-candidate-count-asymmetry)
   - [1.3 QPS and the cost of large models](#13-qps-and-the-cost-of-large-models)
   - [1.4 The tension scaling laws create](#14-the-tension-scaling-laws-create)
2. [Generative-Retrieval Serving: Constrained Decoding over a Trie](#2-generative-retrieval-serving-constrained-decoding-over-a-trie)
   - [2.1 The paradigm shift](#21-the-paradigm-shift)
   - [2.2 Why you need *constrained* decoding](#22-why-you-need-constrained-decoding)
   - [2.3 Why naïve trie traversal is slow on accelerators](#23-why-naïve-trie-traversal-is-slow-on-accelerators)
   - [2.4 STATIC — vectorizing the trie](#24-static--vectorizing-the-trie)
   - [2.5 TrieRec — trie-aware positional encodings](#25-trierec--trie-aware-positional-encodings)
3. [Attention & Architecture Efficiency for Big Rankers](#3-attention--architecture-efficiency-for-big-rankers)
   - [3.1 Cache the invariant user-side computation](#31-cache-the-invariant-user-side-computation)
   - [3.2 Sparsify / shorten attention](#32-sparsify--shorten-attention)
   - [3.3 Skip and compress](#33-skip-and-compress)
   - [3.4 MFU as the headline metric](#34-mfu-as-the-headline-metric)
4. [Custom GPU Kernels: GDPA](#4-custom-gpu-kernels-gdpa)
   - [4.1 The problem fused kernels solve](#41-the-problem-fused-kernels-solve)
   - [4.2 What GDPA is](#42-what-gdpa-is)
   - [4.3 Training vs. inference throughput](#43-training-vs-inference-throughput)
5. [MoE Serving: Inference-Time Load Balancing](#5-moe-serving-inference-time-load-balancing)
   - [5.1 The load-imbalance problem](#51-the-load-imbalance-problem)
   - [5.2 Replicate-and-Quantize (R&Q)](#52-replicate-and-quantize-rq)
...
9. [Pinterest in Practice (2024–2026)](#pinterest-in-practice-20242026)
   - [PinFM — DCAT + int4 PTQ](#pinfm--dcat--int4-ptq)
   - [TransAct V2 — serving full lifelong sequences](#transact-v2--serving-full-lifelong-sequences)
   - [InteractRank — post-dot-product cross features at ~21× fewer FLOPs](#interactrank--post-dot-product-cross-features-at-21-fewer-flops)
