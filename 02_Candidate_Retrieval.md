...
1. [Item-Based Collaborative Filtering (ItemCF)](#1-item-based-collaborative-filtering-itemcf)
   - [1.1 Core Idea](#11-core-idea)
   - [1.2 Interest-Score Prediction](#12-interest-score-prediction)
   - [1.3 Item Similarity](#13-item-similarity)
   - [1.4 The Full ItemCF Retrieval Pipeline](#14-the-full-itemcf-retrieval-pipeline)
2. [Swing Model](#2-swing-model)
   - [2.1 Problem ItemCF Has](#21-problem-itemcf-has)
   - [2.2 Swing's Fix: Down-Weight Cliques](#22-swings-fix-down-weight-cliques)
   - [2.3 Summary](#23-summary)
3. [User-Based Collaborative Filtering (UserCF)](#3-user-based-collaborative-filtering-usercf)
   - [3.1 Core Idea](#31-core-idea)
   - [3.2 Interest-Score Prediction](#32-interest-score-prediction)
   - [3.3 User Similarity](#33-user-similarity)
   - [3.4 Down-Weighting Popular Items (降低热门物品权重)](#34-down-weighting-popular-items-降低热门物品权重)
   - [3.5 The Full UserCF Retrieval Pipeline](#35-the-full-usercf-retrieval-pipeline)
4. [Discrete Feature Processing (One-Hot & Embedding)](#4-discrete-feature-processing-one-hot--embedding)
   - [4.1 One-Hot Encoding](#41-one-hot-encoding)
   - [4.2 Embedding (嵌入)](#42-embedding-嵌入)
   - [4.3 Summary](#43-summary)
5. [Matrix Completion & Approximate Nearest Neighbor (ANN) Search](#5-matrix-completion--approximate-nearest-neighbor-ann-search)
   - [5.1 Matrix Completion Model](#51-matrix-completion-model)
   - [5.2 Training](#52-training)
   - [5.3 Why Matrix Completion Works Poorly in Practice](#53-why-matrix-completion-works-poorly-in-practice)
   - [5.4 Online Serving with Matrix Completion](#54-online-serving-with-matrix-completion)
   - [5.5 Approximate Nearest Neighbor (ANN) Search](#55-approximate-nearest-neighbor-ann-search)
6. [Two-Tower Model: Architecture & Training Methods](#6-two-tower-model-architecture--training-methods)
   - [6.1 Architecture](#61-architecture)
   - [6.2 A Structure That Is NOT a Two-Tower (and why it can't be used for retrieval)](#62-a-structure-that-is-not-a-two-tower-and-why-it-cant-be-used-for-retrieval)
   - [6.3 Three Training Paradigms](#63-three-training-paradigms)
   - [6.4 Summary](#64-summary)
7. [Two-Tower Model: Positive & Negative Samples](#7-two-tower-model-positive--negative-samples)
   - [7.1 Positive Samples (正样本)](#71-positive-samples-正样本)
   - [7.2 Where Negatives Come From in the Pipeline](#72-where-negatives-come-from-in-the-pipeline)
   - [7.3 Simple Negatives (简单负样本)](#73-simple-negatives-简单负样本)
   - [7.4 Hard Negatives (困难负样本)](#74-hard-negatives-困难负样本)
   - [7.5 A Common Mistake (常见的错误)](#75-a-common-mistake-常见的错误)
   - [7.6 Summary](#76-summary)
8. [Two-Tower Model: Online Serving & Model Updates](#8-two-tower-model-online-serving--model-updates)
   - [8.1 Online Retrieval](#81-online-retrieval)
   - [8.2 Full Update vs. Incremental Update (全量更新 vs. 增量更新)](#82-full-update-vs-incremental-update-全量更新-vs-增量更新)
   - [8.3 Summary](#83-summary)
9. [Two-Tower Model + Self-Supervised Learning](#9-two-tower-model--self-supervised-learning)
   - [9.1 Motivation](#91-motivation)
   - [9.2 Recap: Listwise Training of the Main Two-Tower Loss](#92-recap-listwise-training-of-the-main-two-tower-loss)
   - [9.3 The Self-Supervised Idea](#93-the-self-supervised-idea)
   - [9.4 Four Feature Transformations (特征变换)](#94-four-feature-transformations-特征变换)
   - [9.5 Self-Supervised Loss](#95-self-supervised-loss)
   - [9.6 Combined Training](#96-combined-training)
   - [9.7 Summary](#97-summary)
10. [Deep Retrieval](#10-deep-retrieval)
   - [10.1 Core Idea](#101-core-idea)
   - [10.2 Item ↔ Path Structure & Indices](#102-item--path-structure--indices)
   - [10.3 Prediction Model: User's Interest in a Path](#103-prediction-model-users-interest-in-a-path)
   - [10.4 Online Retrieval: User → Path → Item](#104-online-retrieval-user--path--item)
   - [10.5 Beam Search](#105-beam-search)
   - [10.6 Training: Jointly Learn the Network AND the Item Representations](#106-training-jointly-learn-the-network-and-the-item-representations)
   - [10.7 Summary](#107-summary)
11. [Other Retrieval Channels (Geo, Author, Cache)](#11-other-retrieval-channels-geo-author-cache)
   - [11.1 Geolocation Retrieval (地理位置召回)](#111-geolocation-retrieval-地理位置召回)
   - [11.2 Author Retrieval (作者召回)](#112-author-retrieval-作者召回)
   - [11.3 Cache Retrieval (缓存召回)](#113-cache-retrieval-缓存召回)
   - [11.4 Summary](#114-summary)
12. [Exposure Filtering & Bloom Filter](#12-exposure-filtering--bloom-filter)
   - [12.1 The Exposure-Filtering Problem (曝光过滤问题)](#121-the-exposure-filtering-problem-曝光过滤问题)
   - [12.2 Bloom Filter Basics](#122-bloom-filter-basics)
   - [12.3 How It Works](#123-how-it-works)
   - [12.4 False-Positive Probability & Optimal Parameters](#124-false-positive-probability--optimal-parameters)
   - [12.5 The Exposure-Filtering Pipeline](#125-the-exposure-filtering-pipeline)
   - [12.6 Bloom Filter Drawbacks](#126-bloom-filter-drawbacks)
13. [Source Notes & Disagreements](#source-notes--disagreements)
14. [Expanded Reading — Recent Advances (2026)](#expanded-reading--recent-advances-2026)
   - [Farewell to Item IDs (TRM, ByteDance)](#farewell-to-item-ids-trm-bytedance)
   - [Vectorizing the Trie — STATIC (Google DeepMind / YouTube)](#vectorizing-the-trie--static-google-deepmind--youtube)
   - [Trie-Aware Transformers — TrieRec (Ant Group)](#trie-aware-transformers--trierec-ant-group)
   - [Semantic Search at LinkedIn](#semantic-search-at-linkedin)
   - [Embedding-Based Retrieval at Airbnb Search](#embedding-based-retrieval-at-airbnb-search)
15. [Pinterest in Practice (2024–2026)](#pinterest-in-practice-20242026)
   - [Multi-Modal Search at Pinterest](#multi-modal-search-at-pinterest)
   - [PinPoint — Composed Image Retrieval Benchmark](#pinpoint--composed-image-retrieval-benchmark)
