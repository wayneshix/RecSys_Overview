...
2. [LastN Averaging: The Baseline Feature](#2-lastn-averaging-the-baseline-feature)
   - [2.1 Definition](#21-definition)
   - [2.2 Where it applies](#22-where-it-applies)
   - [2.3 Multiple sequences, one per behavior type (小红书 practice)](#23-multiple-sequences-one-per-behavior-type-小红书-practice)
   - [2.4 Two practical refinements (from the audio)](#24-two-practical-refinements-from-the-audio)
   - [2.5 Why averaging is lossy (the motivation for DIN)](#25-why-averaging-is-lossy-the-motivation-for-din)
3. [DIN — Deep Interest Network](#3-din--deep-interest-network)
   - [3.1 Setup](#31-setup)
   - [3.2 The attention computation](#32-the-attention-computation)
   - [3.3 DIN = single-head attention (本质是注意力机制)](#33-din--single-head-attention-本质是注意力机制)
   - [3.4 Why DIN works (geometric intuition)](#34-why-din-works-geometric-intuition)
4. [Simple Average vs. Attention: Where Each Is Allowed](#4-simple-average-vs-attention-where-each-is-allowed)
5. [SIM — Search-based Interest Model](#5-sim--search-based-interest-model)
   - [5.1 The problem with DIN: it can't go long](#51-the-problem-with-din-it-cant-go-long)
   - [5.2 The key insight (how to improve DIN)](#52-the-key-insight-how-to-improve-din)
   - [5.3 The two stages](#53-the-two-stages)
   - [5.4 Step 1 — Search: Hard vs. Soft](#54-step-1--search-hard-vs-soft)
   - [5.5 Step 2 — Attention on the TopK](#55-step-2--attention-on-the-topk)
   - [5.6 Time-interval features (使用时间信息) — a useful trick](#56-time-interval-features-使用时间信息--a-useful-trick)
   - [5.7 SIM paper conclusions (结论)](#57-sim-paper-conclusions-结论)
6. [Key Insights](#6-key-insights)
7. [Expanded Reading — Recent Advances (2026)](#expanded-reading--recent-advances-2026)
   - [ULTRA-HSTU: bending the scaling-law curve with efficient self-attention](#ultra-hstu-bending-the-scaling-law-curve-with-efficient-self-attention)
   - [A Survey of User Lifelong Behavior Modeling (ULBM)](#a-survey-of-user-lifelong-behavior-modeling-ulbm)
   - [Feed SR: an industrial-scale sequential recommender for LinkedIn Feed](#feed-sr-an-industrial-scale-sequential-recommender-for-linkedin-feed)
   - [QARM V2: reasoning-aligned multimodal Semantic IDs for GSU/ESU](#qarm-v2-reasoning-aligned-multimodal-semantic-ids-for-gsuesu)
   - [Scaling Personalization with User Foundation Models](#scaling-personalization-with-user-foundation-models)
8. [Pinterest in Practice (2024–2026)](#pinterest-in-practice-20242026)
   - [TransAct V2 — lifelong sequences attended end-to-end (no GSU/ESU compression)](#transact-v2--lifelong-sequences-attended-end-to-end-no-gsuesu-compression)
